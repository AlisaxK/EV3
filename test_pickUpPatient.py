...
from main import pickupPatientFromWaitingRoom, turn_left_to_rooms


class TestPickupPatient(unittest.TestCase):
    @patch("main.wait_for_phone_removed")
    @patch("main.follow_line_with_green_count", return_value=("TARGET_ROOM_REACHED", 1))
    @patch("main.sensor_ir")
    @patch("main.sensor_right")
    @patch("main.sensor_floor")
    def test_pickup_patient_sends_correct_message(
        ...
        with patch("main.turn_left_90_degrees"), patch(
            "main.wait_for_phone_placed"
        ), patch("main.tank_drive") as mock_drive:
    ...
    @patch("main.wait_for_phone_placed")
    @patch("main.tank_drive")
    @patch("main.sensor_ir")
    @patch("main.sensor_right")
    @patch("main.sensor_floor")
    @patch("main.follow_line_with_green_count", return_value=("TARGET_ROOM_REACHED", 1))
